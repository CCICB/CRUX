test_that("Creation of maf_dataset_wrapper from tcga dataset is successful", {
  valid_maf_filepath <- system.file("test_data/tcga_laml.subsampled.maf.gz" ,package="CRUX")
  
  
  maf_dataset_wrapper <- user_data_filepath_to_class_maf_dataset_wrapper(filepath = valid_maf_filepath, maf_data_pool = new_maf_data_pool(), display_name = "Subsampled TCGA laml", short_name="LAML_Subsampled")
  ...
  expect_error(user_data_filepath_to_class_maf_dataset_wrapper(filepath = "Randompath_ASdlkjadlkjaasda", maf_data_pool = new_maf_data_pool(), display_name = "Subsampled TCGA laml", short_name="LAML_Subsampled"))
  
  
  valid_maf_filepath <- system.file("test_data/tcga_laml.subsubsampled.maf.gz" ,package="CRUX")
  maf_dataset_wrapper <- user_data_filepath_to_class_maf_dataset_wrapper(filepath = valid_maf_filepath, maf_data_pool = new_maf_data_pool(), display_name = "Subsampled TCGA laml", short_name="LAML_Subsampled")
})
