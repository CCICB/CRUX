...
mod_cnv_ui <- function(id){
  ...
  tagList(
    ...
    shinyWidgets::panel(
      heading = "Step 4: Choose Analysis / Visualisation",
      tabsetPanel(
  ...
  moduleServer( id, function(input, output, session){
    ...
    output$out_dir_looks_like_gistic <- renderText({
      ...
      if(directory_looks_like_gistic()){
        ...
        p("Current folder does not look like GISTIC output!", class="alert-danger", gistic_bad_path()) %>% as.character() %>% return()
    })
    ...
    mod_plot_gistic_genome_server("mod_plot_genome", gistic, maf)
