...
#' Check if clinical features are plottable on an Oncoplot
#'
#' Check if clinical features are plottable on an Oncoplot. See details for the rules used
#'
#' @param clinical_annotation a vector that represents a value of clinical annotations
#'
#' @return String describing reason column can't be plotted
#' 
#' @details
#' Rules:
#' 
#' 1. If column is non-nummeric and contains >100 unique elements maftools 'get_anno_cols' throws the error 'Error in sample.int(length(x), size, replace, prob) : 
#' cannot take a sample larger than the population when "replace = FALSE"'
#' 
#'
# maftools_check_clinical_feature_oncoplot_compatibility <- function(maf, clinFeature, ...){
#   
#   samples = maftools_oncoplot_samples_to_be_plotted(maf=maf, ...)
#   
#   df_subset = maf@clinical.data %>%
#     dplyr::filter(Tumor_Sample_Barcode %in% samples)
#   
#   df <- lapply(df_subset, function(vec){
#     type <- if(is.numeric(vec) ) "numeric" else "non-numeric"
#     n_unique <- dplyr::n_distinct(vec)
#     plottable <- !n_unique > 100 | type == "numeric"
#     return(list("type" = type, "levels" = n_unique, "plottable" = plottable))
#     }) %>%
