    ...
    fluidRow(
      shinyWidgets::panel(status = "primary",
        shinyWidgets::pickerInput(
          ...
          label = "Please select a dataset", choices = NULL, selected = character(0), 
...
mod_select_maf_dataset_wrapper_server <- function(id, maf_data_pool, label = "Dataset"){
  ...
  moduleServer( id, function(input, output, session){
    ...
    observe({
      ...
      if(!identical(unique_dataset_names, last_dataset_name())){
        last_dataset_name(unique_dataset_names)
        shinyWidgets::updatePickerInput(session = session, inputId = "in_pick_dataset", selected = character(0), choices = unique_dataset_names)
      }
      #else
    ...
    #   if(input$in_switch_filter_dubious_genes){
...
mod_select_maf_dataset_wrapper_ui2 <- function(id, panel=TRUE){
  ns <- NS(id)
  tagList(
    mod_select_dataset_from_maf_data_pool_pickerinput_ui(
      ns("in_picker_dataset"), panel=panel),
      shinyWidgets::awesomeCheckbox(inputId = ns("in_switch_filter_dubious_genes"), label = "Filter Dubious Genes", value = FALSE),
...
#' 
#' @return maf_dataset_wrapper.
mod_select_maf_dataset_wrapper_server2 <- function(id, maf_data_pool, label = "Dataset"){
  #browser()
  utilitybeltshiny::assert_reactive(maf_data_pool)
